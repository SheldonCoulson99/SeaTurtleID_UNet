...
class ImageToImage2D_kfold(Dataset):
    ...
    def __init__(self, dataset_path: str,
        ...
        self.images_list = [
            item for item in self.images_list if item in filelists]
    ...
    def __getitem__(self, idx):
        ...
        if self.task_name == "Turtle":
            image = cv2.imread(os.path.join(self.input_path, image_filename))
            mask = cv2.imread(os.path.join(self.output_path,
                              image_filename.split('.')[0]+'_mask.png'), 0)

        elif self.task_name == "Turtle_H":
            image = cv2.imread(os.path.join(self.input_path, image_filename))
            mask = cv2.imread(os.path.join(self.output_path,
                              image_filename.split('.')[0]+'_mask.png'), 0)

        elif self.task_name == "Turtle_F":
            image = cv2.imread(os.path.join(self.input_path, image_filename))
            mask = cv2.imread(os.path.join(self.output_path,
                              image_filename.split('.')[0]+'_mask.png'), 0)
