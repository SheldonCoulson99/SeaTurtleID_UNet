...
def main_loop(train_loader, val_loader, batch_size=config.batch_size, model_type='', fold=0, tensorboard=True, kfold=0):
    ...
    if model_type == 'UNet':
        ...
        logger.info('transformer head num: {}'.format(
            config_vit.transformer.num_heads))
        logger.info('transformer head dim: {}'.format(
            config_vit.transformer.embedding_channels))
        logger.info('transformer layers num: {}'.format(
            config_vit.transformer.num_layers))
        logger.info('transformer expand ratio: {}'.format(
            config_vit.expand_ratio))
    ...
    for epoch in range(config.epochs):  # loop over the dataset multiple times
        logger.info('\n========= {} | Fold [{}/{}] | Epoch [{}/{}] ========='.format(
            config.model_name, fold, kfold, epoch + 1, config.epochs + 1))
        logger.info(config.session_name)
        # train for one epoch
        model.train(True)
        logger.info('Training with batch size : {}'.format(batch_size))
        train_one_epoch(train_loader, model, criterion, optimizer,
                        writer, epoch, None, fold, kfold, logger)
        ...
        logger.info('\t early_stopping_count: {}/{}'.format(early_stopping_count,
...
if __name__ == '__main__':
    # set `True' can make the results reproducible, but costs more training time
    deterministic = False
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(filelists)):
        ...
        np.savetxt(config.save_path+"val_fold_" +
                   str(fold+1)+".txt", val_filelists, '%s')
        logger.info("Total Nums: {}, train: {}, val: {}".format(
            len(filelists), len(train_filelists), len(val_filelists)))

        train_tf = RandomGenerator(
            output_size=[config.img_size, config.img_size2])
        ...
        dice = main_loop(train_loader, val_loader, model_type=config.model_name,
                         fold=fold+1, tensorboard=True, kfold=kfold)
